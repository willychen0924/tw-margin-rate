...
- 手機版隱藏底部「維持率／指數」軸標題，左右及下軸刻度使用 `18px` SVG 字級，右軸千位數使用 `k` 縮寫；桌機刻度與底部軸標題均使用 `9px` SVG 字級、`--mmc-muted` 顏色及一般字重，軸標題字距為 `.18em`。水平格線需清楚可辨。圖表支援觸控長按後拖曳十字準線與 tooltip，桌機維持 hover。
...
- 手機版 header band 左側固定上市／櫃買 chip，右側圖例固定為兩欄兩列：第一列維持率／指數，第二列餘額／市值比；名稱與數值不可拆行，兩列間距為 `0`，圖例上下內距為 `1px`，整體維持靠左排列。
- `360px` 以下窄螢幕維持相同 2×2 結構，僅縮小 chip、色塊、圖例字級與欄距；header band 不得額外造成水平溢出。
